...
//
		...
		if( pMap->m_bType == 1 && m_pUserData->m_bNation != zone && (zone < 10 || zone > 21))		// 상대방 국가로 못넘어 가게..
			return;
	}
...
// Also add the weight of items in the inventory....
	for(auto i=0 ; i < HAVE_MAX+SLOT_MAX ; i++)  {
...
void CUser::PartyRemove(int memberid)
{
	...
	for(auto i=0; i<8; i++ ) {			// 파티가 유효한 경우 에는 파티 리스트에서 뺀다.
		if( pParty->uid[i] != -1 ) {
			if( pParty->uid[i] == memberid ) {
				pParty->uid[i] = -1;
				pParty->sHp[i] = 0;
...
//
	...
	if (buff_type) {
		...
		SetSlotItemValue();
		SetUserAbility();
		Send2AI_UserUpdateInfo();	// AI Server에 바데이타 전송....		
...
//	pEventData = pEvent->m_arEvent.GetData(pNpc->m_byEvent);						//
	...
	case NPC_OPERATOR:
		...
		break;
	case 28:
		// int result = GetEventTrigger(TrapNumber);
		// if (result != -1)
		// return result;
		eventid - 1;
		break;
