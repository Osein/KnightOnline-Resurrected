...
void CGameProcNationSelect::Tick()
{
	...
	if (m_pUINationSelectDlg->m_bFadingCompleted) {
...
bool CGameProcNationSelect::ProcessPacket(DataPack* pDataPack, int& iOffset)
{
	...
	switch ( iCmd )										// 커멘드에 다라서 분기..
	{
		case N3_NATION_SELECT:							// 캐릭터 선택 메시지..
		{
			...
			m_pUINationSelectDlg->m_dFadeStartTime = CN3Base::TimeGet();
			m_pUINationSelectDlg->m_dFadeCurrentTime = CN3Base::TimeGet();
			m_pUINationSelectDlg->m_bFadingCompleted = false;
			m_pUINationSelectDlg->m_bFading = true;
			m_pUINationSelectDlg->m_bPreserveFade = true;
		}
		return true;
	}
