...
typedef struct __TABLE_ITEM_EXT // 장착 아이템에 관한 리소스 레코드...
{
	...
	BYTE    bySoulBind;                // 29 소울 바인드 - 일대일시 아이템을 떨구는 정도 - 퍼센트 - 현재는 안쓴다.

	short    siBonusStr;                // 30 힘 보너스
	short    siBonusSta;                // 31 체력 보너스
	short    siBonusDex;                // 32 민첩성 보너스
	short    siBonusInt;                // 33 지능 보너스
	short    siBonusMagicAttak;        // 34 마력 보너스
	short    siBonusHP;                // 35 HP 보너스
	short    siBonusMSP;                // 36 MSP 보너스

	short    siRegistFire;            // 37 불 저항
	short    siRegistIce;            // 38 냉기 저항
	short    siRegistElec;            // 39 전기 저항
	short    siRegistMagic;            // 40 마법 저항
	short    siRegistPoison;            // 41 독 저항
	short    siRegistCurse;            // 42 저주 저항
	
	DWORD    dwEffectID1;            // 43 마법 효과 ID1
	DWORD    dwEffectID2;            // 44 마법 효과 ID2

	short    siNeedLevel;            // 45 요구 레벨        플레이어의 iLeve..
	short    siNeedRank;                // 46 요구 작위        플레이어의 iRank..
	short    siNeedTitle;            // 47 요구 타이틀     플레이어의 iTitle..
	short    siNeedStrength;            // 48 요구 힘         플레이어의 iStrength..
	short    siNeedStamina;            // 49 요구 정력     플레이어의 iStamina..
	short    siNeedDexterity;        // 50 요구 민첩        플레이어의 iDexterity..
	short    siNeedInteli;            // 51 요구 지능        플레이어의 iIntelligence..
	short    siNeedMagicAttack;        // 52 요구 마력        플레이어의 iMagicAttak..
} TABLE_ITEM_EXT;
