											...
											int iItemDurability)
{
	...
	if(szResrcFN.empty()) pPart = m_pChrs[iPosIndex]->PartSet(ePartPos, pLooks->szPartFNs[ePartPos]);
...
void CGameProcCharacterSelect::CharacterSelect()
{
	...
	if ( m_eCurProcess == PROCESS_PRESELECT )
	{
		if ( m_pChrs[iIndex] )
		{
			...
			auto freezeTime = 0.6f;
			m_pChrs[iIndex]->AniCurSet(SELECT_ANIM_SELECTED, true, freezeTime);
			m_pChrs[iIndex]->Tick();

			m_pUICharacterSelect->m_dFadeStartTime = CN3Base::TimeGet();
			m_pUICharacterSelect->m_dFadeCurrentTime = CN3Base::TimeGet();
			m_pUICharacterSelect->m_bFadingCompleted = false;
			m_pUICharacterSelect->m_bFading = true;
			m_pUICharacterSelect->m_bPreserveFade = true;
			m_pUICharacterSelect->m_dFadeAnimationTime = m_pChrs[iIndex]->AniCurTime() + freezeTime;
